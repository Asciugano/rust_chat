...
fn command_dragon(command_name: &str, args: &mut env::Args) -> Result<()> {
    let address = args.next().ok_or_else(|| {
        ...
        eprintln!("[ERROR]: no address is provided. Example: 127.0.0.0:4444");
    })?;

    let token = args.next();

    let mut server = TcpStream::connect(&address).map_err(|err| {
        eprintln!("[ERROR]: colud not connect to {address}: {err}");
    })?;

    if let Some(token) = token {
        println!("[INFO]: Sending token...");
        write!(&server, "{token}").map_err(|err| {
            eprintln!("[ERROR]: colud not authorize with the token: {err}");
        })?;
    }
        ...
        match TcpStream::connect(&address) {
            Ok(conn) => {
                let local_addr = conn.local_addr().map_err(|err| {
                    eprintln!(
                        "[ERROR]: could not get local address of connection to {address}: {err}"
                    );
                })?;
                conns.push(conn);
                eprintln!(
                    "[INFO]: connected to {local_addr}. Opened {n} connections",
                    n = conns.len()
                );
            }
            Err(err) => {
...
fn command_gnome(command_name: &str, args: &mut env::Args) -> Result<()> {
    ...
    loop {
        ...
        eprintln!("[INFO]: connected to {local_addr}. Disconnectiong... ");
    }
}
...
const COMMANDS: &[Command] = &[
    ...
    Command {
        name: "hydra",
        description: "Opens as manu connections as possible",
        run: command_hydra,
    },
    Command {
        name: "gnome",
        description: "Keeps opening ad closign connections",
        run: command_gnome,
    },
];
...
fn main() -> ExitCode {
    ...
    if let Some(command_name) = args.next() {
        ...
        usage(&program);
        eprintln!("[ERROR]: No subcomand is provided");
        ExitCode::FAILURE
    }
}
